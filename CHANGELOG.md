...
### Added
- One-command dataset bootstrap: `common.ensure_opennutrition_tsv` + `scripts/download_data.py`
  fetch the full OpenNutrition dataset (~60 MB zip → ≈269 MB TSV) from the official source and
  unzip it into `src/data/`. Idempotent — skips the download when the TSV is already present. The
  notebook auto-downloads on first run.
- OpenNutrition (ODbL) attribution: an inline credit footer on every rendered meal plan and a
  "Data & attribution" section in the README.
...
[Unreleased]: https://github.com/sheldonlsides/langgraph-agent-guardrails/compare/v0.1.0...HEAD
[0.1.0]: https://github.com/sheldonlsides/langgraph-agent-guardrails/releases/tag/v0.1.0
